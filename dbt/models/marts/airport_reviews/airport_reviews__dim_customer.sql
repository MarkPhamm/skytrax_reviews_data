...
-- dim_customer.sql
-- Customer dimension for the airport review star schema.
-- Grain: one row per unique (customer_name, nationality) combination.
-- Surrogate key: dbt_utils hash of the natural key.
-- Scoped to this mart: the same reviewer appearing in another star schema gets
-- the same key (the hash inputs are identical) but is counted independently.
